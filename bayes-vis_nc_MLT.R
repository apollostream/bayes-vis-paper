...
# From: [Hamiltonian Monte Carlo for Hierarchical Models](https://arxiv.org/pdf/1312.0906.pdf)
# by Michael Betancourt and Mark Girolami
schools_nc_mod <- stan(
  "stan/eight_schools_nc.stan", 
  data = schools_data, 
  seed = 1000,
  pars = "theta_tilde",
  include = FALSE
...
print(schools_nc_mod)

schools_nc_draws <- as.array(schools_nc_mod)
schools_nc_diagnostics <- nuts_params(schools_nc_mod)

trace_div_style <- trace_style_np(
  div_color = "darkorange", 
  div_size = 0.15, 
  div_alpha = 0.4
)
mcmc_trace(
  ...
  pars = c("theta[1]", "tau"),
  transform = list(tau = "log"),
  np = schools_nc_diagnostics,
  np_style = trace_div_style
)
mcmc_trace(
  schools_draws,
  pars = c("theta[8]", "tau"),
...
parcoord_div_style <- parcoord_style_np(
  div_color = "green", 
  div_size = 0.15, 
  div_alpha = 0.4
)
parcoord_schools_nc <-
  mcmc_parcoord(
    ...
    alpha = 0.2,
    np = schools_nc_diagnostics,
    np_style = parcoord_div_style
  ) +
  scale_x_discrete(
    expand = c(0, 0),
    labels = parse(text = dimnames(schools_nc_draws)[[3]])
  )
plot(parcoord_schools_nc)
ggsave(
  filename = "plots/mcmc_parcoord_divs_8school_nc.png", 
  width = 4.5, 
  height = 3.75
)


scatter_div_style <- scatter_style_np(
  div_color = "green", 
  div_size = 2.5, 
  div_alpha = 0.75
)
scatter_schools_nc <-
  mcmc_scatter(
    ...
    transform = list(tau = "log"),
    np = schools_nc_diagnostics,
    np_style = scatter_div_style
  ) +
  labs(x = expression(theta[1]), y = expression(log(tau))) +
  xaxis_title(size = rel(1.25)) +
  yaxis_title(size = rel(1.25))

plot(scatter_schools_nc)
ggsave(
  filename = "plots/mcmc_scatter_divs_8school_nc.png", 
  width = 4.5, 
  height = 3.75
)

schools_nc_draws_df %>% 
  gather(key=parameter,value=theta,starts_with("theta")) %>%
  {
    ggplot(.,aes(y = parameter, x = theta, fill = stat(x > 0))) +
      stat_halfeyeh() +
      ylab("") +
      xlab(expression(theta[i])) +
      geom_vline(xintercept = 0, linetype = "dashed") +
      scale_fill_manual(
        name = expression(theta[i] > 0), 
